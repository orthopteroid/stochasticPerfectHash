...
// orthopteroid@gmail.com, Apache-2.0 license
...
template<class K_, class H_>
struct KeyHash {
    K_ key;
    H_ hash;
};

// for Pearson's hash, the mixtable is the state used to calculate the hash
template<class K_, class H_>
KeyHash<K_, H_> HashInfix(
        const string & keyword, const vector<K_> & mixTable, const size_t pkSize,
        function<H_(const char*, const K_*, const size_t)> fnHash)
{
    H_ hash = fnHash(keyword.c_str(), mixTable.data(), mixTable.size());
    return {static_cast<K_>(hash % pkSize), hash};
}

// for other types of hashes we've bolted on the mixtable as a processing step
template<class K_, class H_>
KeyHash<K_, H_> HashPostfix(
        const string & keyword, const vector<K_> & mixTable, const size_t pkSize,
        function<H_(const char*)> fnHash)
{
    // do some bit counting...
    const K_ mixMask = mixTable.size() -1;
    size_t bits = mixMask;
    size_t mixBits = 0;
    while (bits) { mixBits++; bits >>= 1; }

    // gobble the temp value through the mixtable
    H_ rawHash = fnHash(keyword.c_str());
    H_ hash = 0U;
    H_ tmp = rawHash;
    while (tmp) { hash ^= mixTable[tmp & mixMask]; tmp >>= mixBits; }

    return {static_cast<K_>(hash % pkSize), rawHash};
}
...
struct Pearson8x8
{
    const char* name = "Pearson8x8";
    using Key = uint8_t;
    using Hash = uint8_t;
    using KHType = KeyHash<Key, Hash>;
    static Hash hash(const char* sz, const Key* mixTable, const size_t mixSize)
    {
        const uint8_t mixMask = mixSize -1;
        uint8_t h = ~0;
        while(*sz) h ^= mixTable[ *(sz++) & mixMask ];
        return h;
    }
...
struct CRC32x8
{
    const char* name = "CRC32x8";
    using Key = uint8_t;
    using Hash = uint32_t;
    using KHType = KeyHash<Key, Hash>;
    static Hash hash(const char* sz)
    {
    ...
    KHType operator()(const string & keyword, const vector<Key> & mixTable, const size_t pkSize)
    {
        return HashPostfix<Key, Hash>(keyword, mixTable, pkSize, hash);
    }
};

struct FNV1A32x8
{
    const char* name = "FNV1A32x8";
    using Key = uint8_t;
    using Hash = uint32_t;
    using KHType = KeyHash<Key, Hash>;
    static Hash hash(const char* sz)
    {
        ...
        while(*sz) h = (h ^ (uint32_t)*(sz++)) * 0x01000193;
        return h;
    }
    KHType operator()(const string & keyword, const vector<Key> & mixTable, const size_t pkSize)
    {
...
struct FNV1A64x8
{
    ...
    static Hash hash(const char* sz)
    {
        ...
        while(*sz) h = (h ^ (uint64_t)*(sz++)) * 0x00000100000001B3;
        return h;
    }
    KHType operator()(const string & keyword, const vector<Key> & mixTable, const size_t pkSize)
    {
...
// specializations for printing machinery
static char buf[80];
template<class T> char* val_hex(const T & h) { return const_cast<char*>("unimpl"); }
template<> char* val_hex<uint8_t>(const uint8_t & h) { sprintf(buf,"0x%0" PRIx8, h); return buf; }
template<> char* val_hex<uint32_t>(const uint32_t & h) { sprintf(buf,"0x%0" PRIx32, h); return buf; }
template<> char* val_hex<uint64_t>(const uint64_t & h) { sprintf(buf,"0x%0" PRIx64, h); return buf; }
template<class T> const char* val_type(const T & h) { return "unimpl"; }
template<> const char* val_type<uint8_t>(const uint8_t & h) { return "uint8_t"; }
template<> const char* val_type<uint32_t>(const uint32_t & h) { return "uint32_t"; }
template<> const char* val_type<uint64_t>(const uint64_t & h) { return "uint64_t"; }
...
int main(int argc, char **argv)
{
    ...
    sph_solve<Pearson8x8>(keywords);
    sph_solve<CRC32x8>(keywords);
    sph_solve<FNV1A32x8>(keywords);
    sph_solve<FNV1A64x8>(keywords);
